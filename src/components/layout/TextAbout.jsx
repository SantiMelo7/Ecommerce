export default function TextAbout() {
  return (
    <div className="flex flex-col gap-y-2 mt-2 max-w-screen-md">
      <span className="span-about text-red-500 text-md">
        Welcome to Lorem your one-stop destination for all things tech! Nestled
        in the heart of downtown, Lorem boasts a vast array of cutting-edge
        gadgets and gizmos to satisfy even the most discerning tech enthusiasts.
        From the latest smartphones and sleek laptops to state-of-the-art smart
        home devices and immersive VR gear, we&apos;ve got you covered.
      </span>
      <span className="span-about text-md text-green-200">
        Step into our sleek, modern showroom where knowledgeable staff members
        are on hand to guide you through the latest advancements in technology.
        Whether you&apos;re a seasoned tech guru or just dipping your toes into
        the digital world, our friendly experts are here to help you find the
        perfect device to fit your needs.
      </span>
      <span className="span-about md:flex sm:hidden text-green-500 text-md">
        But we&apos;re not just about selling gadgets at Lorem, we&apos;re
        passionate about fostering a tech-savvy community. That&apos;s why we
        offer regular workshops and events where customers can learn new skills,
        discover emerging trends, and connect with like-minded tech enthusiasts.
      </span>
      <span className="span-about sm:hidden ">
        So whether you&apos;re in the market for a new smartphone, craving the
        latest gaming console, or simply want to geek out over the newest tech
        innovations, come experience the excitement at Lorem where technology
        meets community.
      </span>
    </div>
  );
}
